...
const createDefaultContextStateValue = (
  ...
  props.fieldDefs.forEach(e => {
    fieldDisabled[e.name] =
      e.disabled !== undefined ? e.disabled : false;
    fieldReadOnly[e.name] =
      e.readOnly !== undefined ? e.readOnly : false;
    fieldHidden[e.name] =
      e.isHidden !== undefined ? e.isHidden : false;
  });
  ...
  return {
    ...
    fieldMessage: {},
    defaultFieldRaito: props.defaultFieldRaito
      ? props.defaultFieldRaito
      : '30% 70%',
...
const createReactHookFormProps = (
  props: ContainerProviderProps
): UseFormProps => {
...
const ContainerProvider = forwardRef<
  ContainerRef,
  ContainerProviderProps
>((props, ref: Ref<ContainerRef>) => {
  const fieldRefs = React.useRef<{ [key: string]: any }>({});

  let initialState: ContextState = createDefaultContextStateValue(
    props,
    fieldRefs
  );

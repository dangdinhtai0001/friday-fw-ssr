...
export default function useFilter(
  ...
  const addFilterCriteria2Context = (
    data: IColumnFilterModel
  ): void => {
    const {
      first_operator,
      first_value,
    ...
    if (first_value) {
      let filterCriteria1: FilterCriteria = {
        key: key,
        value: first_value,
        operator: first_operator,
        source: 'COLUMN_1',
      };
      let criteria1AsString =
        convertCriteria2String?.(filterCriteria1);
    ...
    if (second_value) {
      let filterCriteria2: FilterCriteria = {
        key: key,
        value: second_value,
      ...
      let criteria2AsString =
        convertCriteria2String?.(filterCriteria2);
  ...
  const getDisplayedCriteriaAsString = (): string => {
    ...
    let criterias = _.filter(
      filterInstance,
      _.matchesProperty('key', key)
    );

...
function RadioGroup<TValue>(
  props: IRadioGroupProps<TValue>,
  ref: ForwardedRef<IRadioGroupRef>
) {
  const {
    options,
    name = uuidv4(),
  ...
  const renderRadioOption = (
    option: IRadioGroupOptions,
    index: number
  ) => {
    const { id = uuidv4(), label, ...optionGridProps } = option;
      ...
      <Grid key={index} {...optionGridProps}>
        <StyledRadioGroupOption className="styled-radio-group-option">
          <StyledRadioGroupOptionCheckbox
            ...
            disabled={_disabled}
            onChange={e => {
              handleOnChange(e.target.value as TValue);
            }}
            checked={_checked}
          />
          {label && (
